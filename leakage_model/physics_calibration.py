...
Подбор a_ξ и b_ξ по водяным данным (минимизация суммы квадратов по r).
"""
...
# Сетка начальных приближений для multi-start
_INITIAL_GRID = [
    (10.0, -0.7),
    (20.0, -1.5),
    (40.0, -2.5),
...
def calibrate(u1_cal, r_cal, geom_cal, beta,
    ...
    """Калибровка a_ξ и b_ξ по водяным данным.
    ...
    Возвращает
    ----------
    tuple (a_xi, b_xi, metrics_cal, result_cal)
    """
    ...
    def loss(params):
        a_xi, b_xi = params
        res = solve_all(u1_cal, geom_cal, a_xi, b_xi, beta,
    ...
    for a0, b0 in _INITIAL_GRID:
        try:
            opt = minimize(loss, [a0, b0], method="Nelder-Mead",
                           options={"maxiter": 10000, "xatol": 1e-12,
                                    "fatol": 1e-14, "adaptive": True})
            if opt.fun < best_loss:
                best_loss = opt.fun
                best_params = opt.x
                logger.info(f"  Старт ({a0}, {b0}): loss={opt.fun:.6e}, "
    ...
    return a_xi, b_xi, metrics_cal, result_cal
